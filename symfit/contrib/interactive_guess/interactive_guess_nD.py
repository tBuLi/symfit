...
# TODO: Split InteractiveGuessnD and InteractiveGuess2D, and make something
#       intelligent choose either one
...
#            if not hasattr(self.model, 'initial'):
            if not isinstance(self.model, ODEModel):
    ...
    def _make_contour_plot(self, proj, ax):
        """
        ...
        """
        x, y = proj
        x_data = self.independent_data[x.name]
        y_data = self.dependent_data[y.name]
        projected_data = np.column_stack((x_data, y_data)).T
        kde = gaussian_kde(projected_data)

        xx, yy = np.meshgrid(self._x_points[x.name], self._y_points[y.name])
        x_grid = xx.flatten()
        y_grid = yy.flatten()
    ...
    def _update_plot(self, _):
        ...
        for indep_var, dep_var in self._projections:
            ...
            x_vals = self._x_grid[indep_var.name]
            ...
            for x_val in self._x_points[indep_var.name]:
                ...
                y_error = np.percentile(ys, self.percentile)
            ...
            plot_line, caps, error_lines = self._plots[(indep_var, dep_var)]
